...
- Run focused checks with `mise run format-check`, `mise run lint`, and `mise run typecheck`; `mise run check` runs those three and does not run tests.
- Run tests with `mise run test` (`uv run pytest tests`); run one test with `uv run pytest tests/path.py::test_name`.
...
## Tests
- Pytest config lives in `pytest.toml`: async tests/fixtures run under `asyncio_mode = "auto"` and imports use `--import-mode=importlib`.
- `tests/conftest.py` starts a session-scoped `PostgresContainer("postgres:alpine")`; Docker must be available, but the test database connection is generated by testcontainers.
- Tests import `album_tracker_api.main:app`, so `.env` or equivalent env vars from `.env.placeholder` must exist for settings/JWT values even though DB sessions are overridden.
- The app under test and assertions share the same `AsyncSession` via `get_db` override. Endpoint commits can expire ORM objects; store IDs before requests that commit/delete, and requery after the request instead of reading expired attributes.
- When creating test rows directly, prefer `session.add(...)` + `await session.flush()` over `commit()`/`refresh()` unless the test specifically needs a committed transaction. Refreshing parent models can preload empty relationships that handlers then see through the shared session identity map.
- Use `unauthenticated_client` for anonymous/auth endpoints, `client` for a normal authenticated non-admin user, and `admin_client` for write endpoints protected by `get_current_admin_user`.
- API integration tests should use real `AsyncClient` calls and real database state; avoid handler mocks. Validate wrapped response payloads with the relevant Pydantic schema via `Schema.model_validate(response.json()["data"])`.
- Organize endpoint integration tests in classes named by endpoint behavior, e.g. `TestCreateAlbum`, `TestDeleteCard`; use one `test_...` method per success/error path.
