...
- Tests still import `album_tracker_api.main:app`, so `.env` or equivalent env vars from `.env.placeholder` must exist for settings/JWT values even though DB sessions are overridden.
- The `engine` fixture creates/drops all SQLAlchemy metadata around each test; the `session` fixture wraps each test in a transaction and rolls it back.
- Use `client` for authenticated API calls: it creates `test_user` and sets `Authorization: Bearer ...`. Use `unauthenticated_client` for auth endpoints or anonymous requests.
